...
from openff.nagl.molecule._base import (
    BatchMixin,
    MoleculeMixin,
    NAGLMoleculeBase,
)
    ...
    def from_openff(
        ...
        graphs = [
            NXMolHeteroGraph.from_openff(
                offmol, atom_features=atom_features, bond_features=bond_features, toolkit_registry=toolkit_registry
            )
...
class GraphMoleculeBatch(BatchMixin, NAGLMoleculeBase):
    ...
    def unbatch(self) -> list[GraphMolecule]:
        ...
        return [
            ...
            for g, n_repr in zip(
                self.graph.unbatch(self.n_representations_per_molecule), self.n_representations_per_molecule
            )
        ]
