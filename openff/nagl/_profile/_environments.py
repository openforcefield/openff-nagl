...
def check_special_groups(molecule: "Molecule") -> Dict[ChemicalEnvironment, int]:
    ENVIRONMENT_PATTERNS = {
        "[OX2H2]": {ChemicalEnvironment.Aqueous: 1},
        "[NX3H3]": {ChemicalEnvironment.Amine: 1},
    }

    for pattern, group in ENVIRONMENT_PATTERNS.items():
        if molecule.chemical_environment_matches(pattern):
            return dict(group)
    return {}


def analyze_functional_groups(molecule: "Molecule") -> Dict[ChemicalEnvironment, int]:
    import shutil
    import subprocess
    import tempfile
    ...
    if shutil.which("checkmol") is None:
        raise MissingOptionalDependencyError("checkmol")

    groups = check_special_groups(molecule)

    # return if it's a complete match to a known molecule
    if groups:
        return groups
    ...
    for group in result.splitlines():
        ...
        group_environment = CHECKMOL_CODES[group_code[1:]]
