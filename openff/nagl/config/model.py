...
class BaseLayer(ImmutableModel):
    ...
    activation_function: ActivationFunction = Field(description="The activation function to apply for each layer")
    dropout: float = Field(default=0.0, description="The dropout to apply after each layer")
...
class ConvolutionLayer(BaseLayer):
    ...
    aggregator_type: AggregatorType = Field(
        default=None, description="The aggregator function to apply after each convolution"
    )
...
class ConvolutionModule(ImmutableModel):
    architecture: typing.Literal["SAGEConv", "GINConv"] = Field(description="GCN architecture to use")
    layers: list[ConvolutionLayer] = Field(description="Configuration for each layer")


class ReadoutModule(ImmutableModel):
    pooling: typing.Literal["atoms", "bonds"]
    layers: list[ForwardLayer] = Field(description="Configuration for each layer")
    postprocess: PostprocessType | None = Field(description="Optional post-processing layer for prediction")
...
class ModelConfig(ImmutableModel, FromYamlMixin):
    ...
    atom_features: list[DiscriminatedAtomFeatureType] = Field(description="Atom features to use")
    bond_features: list[DiscriminatedBondFeatureType] = Field(
        description=("Bond features to use. Not all architectures support bond features"), default_factory=list
    )
    convolution: ConvolutionModule = Field(description="Convolution config to pass molecular graph through")
    readouts: dict[str, ReadoutModule] = Field(
    ...
    def to_simple_dict(self):
        ...
        """
        dct = self.dict()
        dct["atom_features"] = tuple([{f.feature_name: f.dict(exclude={"feature_name"})} for f in self.atom_features])

        dct["bond_features"] = tuple([{f.feature_name: f.dict(exclude={"feature_name"})} for f in self.bond_features])
        new_dict = dict(dct)
