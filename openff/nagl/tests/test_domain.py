...
class TestChemicalDomain:
    ...
    def test_check_allowed_elements_fail_err(self, openff_methyl_methanoate):
        ...
        allowed, err = domain.check_allowed_elements(molecule=openff_methyl_methanoate, return_error_message=True)
        assert not allowed
    ...
    def test_check_forbidden_patterns_fail_err(self, openff_methyl_methanoate):
        ...
        allowed, err = domain.check_forbidden_patterns(molecule=openff_methyl_methanoate, return_error_message=True)
        assert not allowed
        assert err == "Molecule contains forbidden SMARTS pattern [*:1]~[*:2]"

    def test_check_molecule_err(self, openff_methyl_methanoate):
        domain = ChemicalDomain(allowed_elements=(8, 6, 1), forbidden_patterns=("[*:1]~[*:2]",))
        allowed, err = domain.check_molecule(molecule=openff_methyl_methanoate, return_error_message=True)
        assert not allowed
        assert err == "Molecule contains forbidden SMARTS pattern [*:1]~[*:2]"
