...
class TestGeneralLinearFitTarget:
    def test_single_molecule(self, dgl_methane):
        ...
        labels = {"design_matrix": A_flat, "calculated_vectors": torch.tensor([1, 2, 3, 4, 5])}
        ...
        evaluated = target.evaluate_target(dgl_methane, labels, predictions, {})
        assert torch.allclose(evaluated, torch.tensor([9.5, 29.0, 48.5, 68.0, 87.5]))
    ...
    def test_multiple_molecules(self, dgl_batch):
        ...
        evaluated = target.evaluate_target(dgl_batch, labels, predictions, {})
